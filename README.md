...
Restrict SSH port `22` to your own IP whenever possible. After importing the client configuration, restrict or disable the subscription port `8080`. Never share your subscription URL, client configuration, or any credentials generated by RayLink. Anyone with them can connect to your server.  Watch your cloud bill and bandwidth.
...
请尽可能将 SSH 端口 `22` 的访问权限仅限制为您个人的 IP 地址。在导入客户端配置后，请限制或直接禁用 `8080` 订阅端口。切勿向他人泄露您的订阅链接、客户端配置，或由 RayLink 生成的任何认证凭据，因为任何获取到这些信息的人均可直接连接至您的服务器。注意云服务器账单和流量。
