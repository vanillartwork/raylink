...
| [terminal](docs/terminal.md) | Terminal node — install, options, managing, health check |
| [relay](docs/relay.md) | Relay node — model, upstream parameters, firewall |
...
Restrict SSH port `22` to your own IP whenever possible. After importing the client configuration, restrict or disable the subscription port `8080`. Never share your subscription URL, client configuration, or any credentials generated by RayLink. Anyone with them can connect to your server.  Proxy services generate continuous network traffic. Please monitor your cloud server bandwidth usage closely and be alert to any abnormal traffic spikes. It is recommended to set up traffic monitoring and overage alerts with your cloud provider to avoid unexpected high charges caused by exceeding your quota or node abuse.
...
请尽可能将 SSH 端口 `22` 的访问权限仅限制为您个人的 IP 地址。在导入客户端配置后，请限制或直接禁用 `8080` 订阅端口。切勿向他人泄露您的订阅链接、客户端配置，或由 RayLink 生成的任何认证凭据，因为任何获取到这些信息的人均可直接连接至您的服务器。代理服务会产生持续的网络流量，请密切关注您的云服务器流量使用情况，并警惕任何异常的流量激增。建议在您的云服务提供商处设置流量监控和超额预警，以避免因超出套餐额度或节点被滥用而产生高昂的意外账单。
