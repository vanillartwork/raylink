...
- **客户端可直接导入的订阅** — 通过 HTTP 提供 Clash/Mihomo YAML 和通用 URI-list，
  客户端导入一个 URL 即可。
- **自愈** — 定时自检会重新检测公网 IP、执行端到端 Reality 自测，并在某个 Reality
  target 失效时自动 fallback 切换。
- **Relay 中转** — `客户端 → Relay → Terminal`，用于隐藏出口节点或稳定不佳的链路。
- **下载更稳** — 带重试、超时，以及 `GITHUB_URL_PREFIX` / `XRAY_DOWNLOAD_URL`
  逃生口，应对 GitHub 慢或被墙。
- **IPv4 与 IPv6** — 自动识别地址族，并自动给 IPv6 在链接/URL 中加方括号。
- **幂等且可配置** — 可安全重复运行；所有默认值都能通过环境变量覆盖。
...
| [docs/terminal.md](docs/terminal.md) | Terminal 节点 — 安装、选项、管理、自检 |
| [docs/relay.md](docs/relay.md) | Relay 节点 — 模型、upstream 参数、防火墙 |
