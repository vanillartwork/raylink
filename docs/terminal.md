...
/etc/systemd/system/raylink-terminal-healthcheck.{service,timer}
```
...
```bash
sudo env RESET_REALITY_CREDENTIALS=true raylink terminal   # regenerate Reality keys
sudo env RESET_SUB_TOKEN=true raylink terminal             # regenerate sub token only
```

## Health check

A systemd timer runs `raylink terminal --health-check` (10 min after boot, then
